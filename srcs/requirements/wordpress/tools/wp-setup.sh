...
# Wait until MariaDB is fully up and accepting connections before proceeding.
until mariadb-admin ping -h"mariadb" -u"$MYSQL_USER" -p"$(cat /run/secrets/db_user_password)" --silent; do
    echo "Waiting for MariaDB..."
    sleep 2
done

# Perform fresh WordPress installation if wp-config.php does not exist.
if [ ! -f "/var/www/html/wp-config.php" ]; then
    rm -rf /var/www/html/*
    
    php -d memory_limit=1G /usr/local/bin/wp core download \
        --allow-root \
        --path=/var/www/html
...
# Wait until Redis is fully up and accepting connections.
...
# Install and configure the Redis object cache plugin if it is not already active.
...
# Apply Redis configuration variables to wp-config.php.
...
# Enable the Redis cache bridge.
php -d memory_limit=512M /usr/local/bin/wp redis enable --allow-root --path=/var/www/html

# Enforce correct ownership and permissions for the web server to serve files safely.
...
# completely destroy this running shell script and replace it with the php-fpm daemon. 
# This promotes php-fpm directly to PID 1, ensuring the service stays natively locked 
